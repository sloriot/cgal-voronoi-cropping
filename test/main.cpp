...
int main()
{
  ...
  output.open("hds.cgal");
  for( HDS::Face_iterator fit=hds.faces_begin();fit!=hds.faces_end();++fit)
  {
    std::stringstream sstream;
    HDS::Halfedge_handle curr=fit->halfedge(), end=curr;
    int i=0;
    CGAL_assertion( curr!= HDS::Halfedge_handle() );
    do{
      CGAL_assertion( curr!= HDS::Halfedge_handle() );
